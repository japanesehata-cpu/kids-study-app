...
describe('generateMissingOperandQuestion', () => {
  it('addition Lv1 stays within the 8-20 target band, never shows the visual, and is internally consistent', () => {
    const seenBlanks = new Set<string>()
    for (let i = 0; i < 300; i++) {
      const q = generateMissingOperandQuestion(1, 'addition')
      seenBlanks.add(q.blank!)
      expect(q.showVisual).toBe(false)
      expect(q.blank).toBeDefined()
      expect(q.category).toBe('missingOperandAddition')
      expect(q.operator).toBe('addition')
  ...
  it('subtraction Lv1 stays within the 8-20 target band, never shows the visual, and is internally consistent', () => {
    const seenBlanks = new Set<string>()
    for (let i = 0; i < 300; i++) {
      const q = generateMissingOperandQuestion(1, 'subtraction')
      seenBlanks.add(q.blank!)
      expect(q.showVisual).toBe(false)
      expect(q.blank).toBeDefined()
      expect(q.category).toBe('missingOperandSubtraction')
      expect(q.operator).toBe('subtraction')
      expect(q.answer).toBe(q.operandA - q.operandB)
  ...
  it('Lv2 always uses round-tens components (20-100 range), never shows the visual', () => {
    for (const operator of ['addition', 'subtraction'] as const) {
      for (let i = 0; i < 150; i++) {
        const q = generateMissingOperandQuestion(2, operator)
        expect(q.showVisual).toBe(false)
