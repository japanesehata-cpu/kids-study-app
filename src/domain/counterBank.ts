 ...
 * "place"/"bodyPart" entries in generate-word-images-local.py), house (けん),
 * birthdaycake (さい, lit candle standing in for "age" since a number alone isn't a
 * countable object — exactly one candle per cake, not several, for the same tiling
 * reason as shoe below), and shoe (そく — wordBank's own `shoes` word is correctly a
 * *pair* since the English word is plural, but tiling a pair image N times would show 2N
 * shoe shapes for a count of N; そく technically counts pairs of footwear, so a single
 * shoe per copy is a deliberate simplification favoring on-screen countability over
 * literal unit accuracy). つぶ uses `blueberry` rather than `pea` or `grape` — a pea pod
 * or a grape bunch photographs as one cluster, not individually countable grains, which
 * is exactly what つぶ needs to show. */
