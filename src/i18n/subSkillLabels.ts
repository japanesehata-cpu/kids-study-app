import type { DictionaryKey } from './dictionary'

export const subSkillLabelKey: Record<string, DictionaryKey> = {
  'addition-no-carry': 'subSkillAdditionNoCarry',
  'addition-carry': 'subSkillAdditionCarry',
  'addition-tens': 'subSkillAdditionTens',
  'addition-twenties': 'subSkillAdditionTwenties',
  // No longer produced by the generator (every level shows visuals now), but kept
  // mapped so a subSkill tag saved to localStorage before this change can still resolve
  // to a label instead of crashing the results breakdown — same class of bug just fixed
  // for addition's showVisual field.
  'subtraction-visual': 'subSkillSubtractionVisual',
  'subtraction-basic': 'subSkillSubtractionBasic',
  'subtraction-extended': 'subSkillSubtractionExtended',
  'subtraction-borrow': 'subSkillSubtractionBorrow',
  'subtraction-tens': 'subSkillSubtractionTens',
  'subtraction-twenties': 'subSkillSubtractionTwenties',
  'missing-operand-addition': 'subSkillMissingOperandAddition',
  'missing-operand-subtraction': 'subSkillMissingOperandSubtraction',
  'word-animal': 'subSkillWordAnimal',
  // 'fruit' was folded into the broader 'food' category below; kept mapped so an older
  // saved subSkill tag still resolves instead of crashing the results breakdown.
  'word-fruit': 'subSkillWordFruit',
  'word-food': 'subSkillWordFood',
  'word-nature': 'subSkillWordNature',
  'word-color': 'subSkillWordColor',
  'word-vehicle': 'subSkillWordVehicle',
  'word-clothing': 'subSkillWordClothing',
  'word-household': 'subSkillWordHousehold',
  'word-school': 'subSkillWordSchool',
  'word-weather': 'subSkillWordWeather',
  'word-bodyPart': 'subSkillWordBodyPart',
  'word-toy': 'subSkillWordToy',
  'word-sport': 'subSkillWordSport',
  'word-instrument': 'subSkillWordInstrument',
  'word-shape': 'subSkillWordShape',
  'word-place': 'subSkillWordPlace',
  // englishSentence's per-content-category subSkills — see questionGenerators/
  // englishSentence.ts's `sentence-${category}` tags.
  'sentence-color': 'subSkillSentenceColor',
  'sentence-animal': 'subSkillSentenceAnimal',
  'sentence-vehicle': 'subSkillSentenceVehicle',
  'sentence-food': 'subSkillSentenceFood',
  'sentence-shape': 'subSkillSentenceShape',
  'sentence-clothing': 'subSkillSentenceClothing',
  'sentence-household': 'subSkillSentenceHousehold',
  'sentence-school': 'subSkillSentenceSchool',
  'sentence-weather': 'subSkillSentenceWeather',
  'sentence-bodyPart': 'subSkillSentenceBodyPart',
  'sentence-sport': 'subSkillSentenceSport',
  'sentence-instrument': 'subSkillSentenceInstrument',
  'sentence-toy': 'subSkillSentenceToy',
  'sentence-nature': 'subSkillSentenceNature',
  'logic-pattern': 'subSkillLogicPattern',
  'logic-oddOneOut': 'subSkillLogicOddOneOut',
  'logic-compare': 'subSkillLogicCompare',
  'sudoku-fill': 'subSkillSudokuFill',
  'hiragana-vowels': 'subSkillHiraganaVowels',
  'hiragana-basic': 'subSkillHiraganaBasic',
  'hiragana-mid': 'subSkillHiraganaMid',
  'hiragana-advanced': 'subSkillHiraganaAdvanced',
  'hiragana-youon': 'subSkillHiraganaYouon',
  'katakana-vowels': 'subSkillKatakanaVowels',
  'katakana-basic': 'subSkillKatakanaBasic',
  'katakana-mid': 'subSkillKatakanaMid',
  'katakana-advanced': 'subSkillKatakanaAdvanced',
  'katakana-youon': 'subSkillKatakanaYouon',
  'katakana-gairaigo': 'subSkillKatakanaGairaigo',
  'kanji-numbers': 'subSkillKanjiNumbers',
  'kanji-nature': 'subSkillKanjiNature',
  'kanji-people-body': 'subSkillKanjiPeopleBody',
  'kanji-color-size': 'subSkillKanjiColorSize',
  'kanji-animals-places-study': 'subSkillKanjiAnimalsPlacesStudy',
  'money-recognize': 'subSkillMoneyRecognize',
  'money-combine-2': 'subSkillMoneyCombine2',
  'money-combine-3': 'subSkillMoneyCombine3',
  'alphabet-uppercase': 'subSkillAlphabetUppercase',
  'alphabet-lowercase': 'subSkillAlphabetLowercase',
  'alphabet-case-match': 'subSkillAlphabetCaseMatch',
  'clock-oclock': 'subSkillClockOclock',
  'clock-half': 'subSkillClockHalf',
  'clock-quarter': 'subSkillClockQuarter',
  'clock-fiveminute': 'subSkillClockFiveMinute',
  'clock-anyminute': 'subSkillClockAnyMinute',
  'spot-obvious': 'subSkillSpotObvious',
  'spot-similar': 'subSkillSpotSimilar',
  'spot-subtle': 'subSkillSpotSubtle',
  'counter-word': 'subSkillCounterWord',
}
